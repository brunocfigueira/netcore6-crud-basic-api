...
[ApiController]
[Route("[controller]")]
public class UsersController : ControllerBase, ICrudController<CreateUserDto, UpdateUserDto, ReadUserDto>
{
    ...
    public IActionResult Create([FromBody] CreateUserDto resquest)
    {
        try
        {
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status200OK)]
    public IActionResult Update(long id, [FromBody] UpdateUserDto resquest)
    {
        try
        {
    ...
    /// </summary>
    /// <param name="id">PK record</param>
    /// <param name="resquest">Object for update a password</param>
    /// <returns>IActionResult</returns>
    /// <response code="400">Request has errors</response>
    /// <response code="403">Request was forbidden</response>
    /// <response code="404">Request was not found</response>
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status200OK)]
    public IActionResult ChangePassword(long id, [FromBody] ChangePasswordUserDto resquest)
    {
        try
        {
            return _service.ChangePassword(id, resquest) ? Ok(ResponseMessage.UpdatedSuccess()) : NotFound(ResponseMessage.NotFound());
        }
        catch (ForbiddenAccessException ex)
        {
            return StatusCode(StatusCodes.Status403Forbidden, ResponseForbidden.EmitMessage(ex.Message));
    ...
    [HttpGet("search")]
    [ProducesResponseType(StatusCodes.Status200OK)]
    public IEnumerable<ReadUserDto> Search([FromQuery] int skip = 0, [FromQuery] int take = 10)
    {
